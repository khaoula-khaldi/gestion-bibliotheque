...
<body class="bg-gray-50 text-slate-900 font-sans">
    ...
    <main class="flex-1 p-10 bg-white min-h-screen">
            ...
            <a href="{{ route('auteurs.index') }}" class="text-[10px] font-bold  tracking-[2px] text-gray-400 ">
                ← Retour à la liste
            </a>
                ...
                <form action="{{ isset($auteur) ? route('auteurs.update', $auteur->id) : route('auteurs.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-2 gap-8 mb-8">
                        <div>
                            <label class="block text-[10px]text-gray-600 mb-2">Nom</label>
                            <input type="text" name="nom" value="{{ $auteur->nom ?? old('nom') }}" 
                                class="w-full border-2 border-black px-4 py-3  text-sm" required>
                        </div>
                        <div>
                            <label class="block text-[10px]text-gray-600 mb-2">Prénom</label>
                            <input type="text" name="prenom" value="{{ $auteur->prenom ?? old('prenom') }}" 
                                class="w-full border-2 border-black px-4 py-3text-sm" required>
                        </div>
                    </div>

                    <div class="mb-8">
                        <label class="block text-[10px]text-gray-600 mb-2">Date de Naissance</label>
                        <input type="date" name="date_naissance" value="{{ $auteur->date_naissance ?? old('date_naissance') }}" 
                            class="w-full border-2 border-black px-4 py-3  text-sm">
                    </div>

                    <div class="mb-10">
                        <label class="block text-[10px]text-gray-600 mb-2">Biographie</label>
                        <textarea name="biographie" rows="5" 
                            class="w-full border-2 border-black px-4 py-3  placeholder-gray-300" 
